...
// Calculate the length of the flowData.nodes array
 let nodesArray = [];
   ...
   return axios.post(webhookUrl,     { nodes: flowDataObj.nodes, shortId: flowDataObj.shortId, tenantId: flowDataObj.tenantId, trigger_output:  generateShortIds(flowDataObj.shortId) }
, {
...
flowDataObj.shortId = shortId;
flowDataObj.tenantId = req.tenantId;
 if (!flowDataObj.nodes[1].inputParameters) {
            flowDataObj.nodes[1].inputParameters = {};
        }
flowDataObj.nodes[1].inputParameters.body.content.messages = [{
            role: "user",
            content: webhookBody // Assign the entire webhook body to a single message's content
        }];
 let nodesArray = [];
    if (Array.isArray(flowDataObj.nodes)) {
        nodesArray = flowDataObj.nodes;
